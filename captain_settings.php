<?php
    ...
    if(isset($_POST['update']))
    {
    	...
    	if($count > 0)
    	{
    		...
    		{
    			if($data['passwordd'] == sha1(md5($oldPassword)))
    			{
    				if($newPassword === $confirmPassword)
    				{
    					$hashedpassword = sha1(md5($newPassword));
    					mysqli_query($connect, "UPDATE add_user SET passwordd = '$hashedpassword' WHERE email = '$email'");
					...
					<div class="col-md-12">
						...
						<div class="below-toggle-content">
							...
							<div class="top-part mb-3">
								...
								<a href="captain_dashboard.php" class="d-inline text-dark mt-2 float-left" style="text-decoration: none; float: right; font-weight: 500;"> داشبۆڕد <i class="fas fa-tachometer-alt"></i></a>
							</div>
							<div class="row">
									...
									<form method="POST" action="" class="w-100 p-3 shadow-lg rounded">
									  	...
									  	<button type="submit" name="cancel" class="btn btn-danger">پاشگەزبوونەوە</button>
									</form>
								</div>
							</div>
						</div>
